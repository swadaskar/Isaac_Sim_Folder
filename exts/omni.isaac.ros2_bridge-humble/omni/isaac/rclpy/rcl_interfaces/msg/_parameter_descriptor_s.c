...
bool rcl_interfaces__msg__parameter_descriptor__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // dynamic_typing
    PyObject * field = PyObject_GetAttrString(_pymsg, "dynamic_typing");
    if (!field) {
      return false;
    }
    assert(PyBool_Check(field));
    ros_message->dynamic_typing = (Py_True == field);
    Py_DECREF(field);
  }
...
PyObject * rcl_interfaces__msg__parameter_descriptor__convert_to_py(void * raw_ros_message)
{
  ...
  {  // dynamic_typing
    PyObject * field = NULL;
    field = PyBool_FromLong(ros_message->dynamic_typing ? 1 : 0);
    {
      int rc = PyObject_SetAttrString(_pymessage, "dynamic_typing", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
