"""Support for simplified access to data on nodes of type omni.isaac.ros2_bridge.ROS2PublishClock
...
import carb
class OgnROS2PublishClockDatabase(og.Database):
    """Helper class providing simplified access to data on nodes of type omni.isaac.ros2_bridge.ROS2PublishClock
    ...
    class ValuesForInputs(og.DynamicAttributeAccess):
        ...
        def __init__(self, node: og.Node, attributes, dynamic_attributes: og.DynamicAttributeInterface):
            ...
            self._batchedReadValues = [0, None, "", 10, 0, "clock"]
    ...
    def __init__(self, node):
        ...
        self.inputs = OgnROS2PublishClockDatabase.ValuesForInputs(node, self.attributes.inputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_OUTPUT)
        self.outputs = OgnROS2PublishClockDatabase.ValuesForOutputs(node, self.attributes.outputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_STATE)
        self.state = OgnROS2PublishClockDatabase.ValuesForState(node, self.attributes.state, dynamic_attributes)
