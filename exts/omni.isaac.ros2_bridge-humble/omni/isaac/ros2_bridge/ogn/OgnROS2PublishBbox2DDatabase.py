"""Support for simplified access to data on nodes of type omni.isaac.ros2_bridge.ROS2PublishBbox2D
...
import carb
import numpy
class OgnROS2PublishBbox2DDatabase(og.Database):
    """Helper class providing simplified access to data on nodes of type omni.isaac.ros2_bridge.ROS2PublishBbox2D
    ...
    INTERFACE = og.Database._get_interface([
        ...
        ('inputs:topicName', 'string', 0, None, 'Name of ROS2 Topic', {ogn.MetadataKeys.DEFAULT: '"bbox2d"'}, True, 'bbox2d', False, ''),
    ])
    @classmethod
    ...
    class ValuesForInputs(og.DynamicAttributeAccess):
        ...
        def __init__(self, node: og.Node, attributes, dynamic_attributes: og.DynamicAttributeInterface):
            ...
            self._batchedReadValues = [0, None, "sim_camera", "", 10, 0.0, "bbox2d"]
    ...
    def __init__(self, node):
        ...
        self.inputs = OgnROS2PublishBbox2DDatabase.ValuesForInputs(node, self.attributes.inputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_OUTPUT)
        self.outputs = OgnROS2PublishBbox2DDatabase.ValuesForOutputs(node, self.attributes.outputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_STATE)
        self.state = OgnROS2PublishBbox2DDatabase.ValuesForState(node, self.attributes.state, dynamic_attributes)
