...
class Extension(omni.ext.IExt):
    def on_startup(self, ext_id):
        ...
        if "omni.isaac.ros2_bridge" == ext_id.rsplit("-", 1)[0]:
            ext_manager.set_extension_enabled("omni.isaac.ros2_bridge", False)
            carb.log_error(f"{ext_id} bridge extension cannot be enabled if omni.isaac.ros2_bridge-humble is enabled")
            return

        self._extension_path = ext_manager.get_extension_path(ext_id)
        for b in ["omni.isaac.ros_bridge", "omni.isaac.ros2_bridge", "omni.isaac.ros2_bridge-humble"]:
            if b != "omni.isaac.ros2_bridge-humble" and ext_manager.is_extension_enabled(b):
        ...
        try:
            carb.get_framework().load_plugins(
                loaded_file_wildcards=["omni.isaac.ros2_humble_bridge.plugin"],
                search_paths=[os.path.abspath(os.path.join(self._extension_path, "bin"))],
            )
            from omni.isaac.ros2_bridge import _ros2_humble_bridge

            self._module = _ros2_humble_bridge
            self._ros2bridge = self._module.acquire_ros2_bridge_interface()
        except Exception as e:
            carb.log_error(e)
            carb.log_error(
                "Cannot load ROS2 humble bridge after loading ROS2 bridge, please restart Isaac Sim and only enable/use one of the ROS2 bridges"
            )
            ext_manager.set_extension_enabled("omni.isaac.ros2_bridge-humble", False)
            return
