...
from omni.isaac.onshape.onshape_client.oas.exceptions import ApiKeyError, ApiTypeError, ApiValueError
...
class OpenApiModel(object):
    ...
    def set_attribute(self, name, value):
        ...
        if name in openapi_types:
            ...
            # Should be forward compatible to endpoint changes.
            return
        elif self.additional_properties_type is not None:
            required_types_mixed = self.additional_properties_type

        # Can be None if not a required property to allow for null field serialization
        if name not in self.required_properties:
            required_types_mixed = required_types_mixed + (type(None),)
    ...
    def get_subtypes(cls):
        """Find all subtypes of this type"""
        if not cls.discriminator():
            return []
        subtypes = []
        for discriminator_key, discriminator_object in cls.discriminator().items():
            for v, subtype in discriminator_object.items():
                subtypes.append(subtype)
                subtypes = subtypes + subtype.get_subtypes()
        return subtypes
...
class ModelSimple(OpenApiModel):
    ...
    def __getattr__(self, name):
        ...
        if self._path_to_item:
            path_to_item.extend(self._path_to_item)
        path_to_item.append(name)
        raise ApiKeyError("{0} has no key '{1}'".format(type(self).__name__, name), [name])
    ...
    def __eq__(self, other):
        ...
        if not six.PY3 and len(types) == 2 and unicode in types:  # noqa: F821
            vals_equal = this_val.encode("utf-8") == that_val.encode("utf-8")
        if not vals_equal:
            return False
        return True
...
class ModelComposed(OpenApiModel):
    ...
    def __setattr__(self, name, value):
        ...
        if self._path_to_item:
            path_to_item.extend(self._path_to_item)
        path_to_item.append(name)
        raise ApiKeyError("{0} has no key '{1}'".format(type(self).__name__, name), path_to_item)
...
def get_simple_class(input_value):
    ...
    if isinstance(input_value, type):
        ...
        return date
    elif six.PY2 and isinstance(input_value, (str_py2, unicode_py2, str)) or isinstance(input_value, str):
        return str
...
def check_validations(validations, input_variable_path, input_values):
    ...
    if "max_length" in current_validations and len(input_values) > current_validations["max_length"]:
        raise ApiValueError(
            "Invalid value for `%s`, length must be less than or equal to "
            "`%s`" % (input_variable_path[0], current_validations["max_length"])
        )

    if "min_length" in current_validations and len(input_values) < current_validations["min_length"]:
        raise ApiValueError(
            "Invalid value for `%s`, length must be greater than or equal to "
            "`%s`" % (input_variable_path[0], current_validations["min_length"])
        )

    if "max_items" in current_validations and len(input_values) > current_validations["max_items"]:
        raise ApiValueError(
            "Invalid value for `%s`, number of items must be less than or "
            "equal to `%s`" % (input_variable_path[0], current_validations["max_items"])
        )

    if "min_items" in current_validations and len(input_values) < current_validations["min_items"]:
        raise ValueError(
    ...
    if "exclusive_maximum" in current_validations and max_val >= current_validations["exclusive_maximum"]:
        raise ApiValueError(
            "Invalid value for `%s`, must be a value less than `%s`"
            % (input_variable_path[0], current_validations["exclusive_maximum"])
        )

    if "inclusive_maximum" in current_validations and max_val > current_validations["inclusive_maximum"]:
        raise ApiValueError(
            "Invalid value for `%s`, must be a value less than or equal to "
            "`%s`" % (input_variable_path[0], current_validations["inclusive_maximum"])
        )

    if "exclusive_minimum" in current_validations and min_val <= current_validations["exclusive_minimum"]:
        raise ApiValueError(
            "Invalid value for `%s`, must be a value greater than `%s`"
            % (input_variable_path[0], current_validations["exclusive_maximum"])
        )

    if "inclusive_minimum" in current_validations and min_val < current_validations["inclusive_minimum"]:
        raise ApiValueError(
    ...
    if "regex" in current_validations and not re.search(
        current_validations["regex"]["pattern"], input_values, flags=flags
    ):
        raise ApiValueError(
            r"Invalid value for `%s`, must be a follow pattern or equal to "
            r"`%s` with flags=`%s`" % (input_variable_path[0], current_validations["regex"]["pattern"], flags)
        )
...
def remove_uncoercible(required_types_classes, current_item, from_server, must_convert=True):
...
def change_keys_js_to_python(input_dict, model_class):
    ...
    output_dict = {}
    reversed_attr_map = {value: key for key, value in six.iteritems(model_class.attribute_map)}
...
def deserialize_primitive(data, klass, path_to_item):
    ...
    try:
        ...
        raise ApiValueError(
            "{0}Failed to parse {1} as {2}".format(additional_message, repr(data), get_py3_class_name(klass)),
...
def fix_model_input_data(model_data, model_class):
    ...
    fixed_model_data = change_keys_js_to_python(model_data, model_class)
    if model_class._composed_schemas() is not None:
        for allof_class in model_class._composed_schemas()["allOf"]:
            fixed_model_data = change_keys_js_to_python(fixed_model_data, allof_class)
    return fixed_model_data


def deserialize_model(model_data, model_class, path_to_item, check_type, configuration, from_server):
    ...
    kw_args = dict(
        _check_type=check_type, _path_to_item=path_to_item, _configuration=configuration, _from_server=from_server
    )

    used_model_class = model_class
    if model_class.discriminator() is not None:
        used_model_class = get_discriminator_class(model_class, from_server, model_data)
    ...
    if isinstance(model_data, dict):
        fixed_model_data = change_keys_js_to_python(model_data, used_model_class)
...
def recursive_discriminator(model_class):
    """
    A model can return a new model using it's discriminator.
    But if that new model also has a discriminator we might have
    not yet reached our final model. Keep recursively calling
    until we build up the discriminator to all the possible subtypes.
    This 'enhanced recursive discriminator' is stored in a simple module level
    in memory cache for performant deserialization.
    """
...
def get_discriminator_class(model_class, from_server, data):
    """Returns the child class specified by the discriminator"""
    discriminator = recursive_discriminator(model_class)
...
def attempt_convert_item(
    ...
    valid_classes_coercible = remove_uncoercible(valid_classes_ordered, input_value, from_server)
...
def validate_and_convert_types(
    input_value, required_types_mixed, path_to_item, from_server, _check_type, configuration=None
):
    ...
    # See if this matches subtypes
    if not valid_type:
        subclasses = []
        for valid_class in valid_classes:
            get_subtypes_function = getattr(valid_class, "get_subtypes", None)
            if callable(get_subtypes_function):
                subclasses.append(valid_class.get_subtypes())
        flattened_subclasses = [subclass for subclass_list in subclasses for subclass in subclass_list]
        valid_type = input_class_simple in set(flattened_subclasses)
    ...
    if len(valid_classes) > 1 and configuration:
        ...
        valid_classes_coercible = remove_uncoercible(valid_classes, input_value, from_server, must_convert=False)
    ...
    if isinstance(input_value, list):
        ...
        for inner_key, inner_val in six.iteritems(input_value):
            ...
            input_value[inner_key] = validate_and_convert_types(
                inner_val, inner_required_types, inner_path, from_server, _check_type, configuration=configuration
            )
    return input_value
...
def model_to_dict(model_instance, serialize=True):
    ...
    original_instance = model_instance
    ...
    for model_instance in model_instances:
        for attr, value in six.iteritems(model_instance._data_store):
            ...
            if isinstance(value, list):
                result[attr] = list(
                    map(lambda x: model_to_dict(x, serialize=serialize) if hasattr(x, "_data_store") else x, value)
                )
        ...
        # Auto add discriminator value if not already set.
        discriminator = model_instance.discriminator()
        if discriminator is not None:
            for d_key, d_val in discriminator.items():
                # Only set if this won't override a user-set property
                if d_key not in original_instance._data_store.keys():
                    for d_type, d_type_value in d_val.items():
                        if isinstance(original_instance, d_type_value):
                            # Convert to server name
                            d_key = original_instance.attribute_map.get(d_key, d_key)
                            result[d_key] = d_type
                            break
    return result
...
def get_allof_instances(self, model_args, constant_args):
    ...
    for allof_class in self._composed_schemas()["allOf"]:
        ...
        fixed_model_args = change_keys_js_to_python(model_args, allof_class)
...
def get_oneof_instance(self, model_args, constant_args):
    ...
    for oneof_class in self._composed_schemas()["oneOf"]:
        # transform js keys to python keys in fixed_model_args
        fixed_model_args = change_keys_js_to_python(model_args, oneof_class)
    ...
    if len(oneof_instances) == 0:
        raise ApiValueError(
            ...
            "make any instances of the classes in oneOf definition." % self.__class__.__name__
        )
    elif len(oneof_instances) > 1:
        raise ApiValueError(
            "Invalid inputs given to generate an instance of %s. Multiple "
            "oneOf instances were generated when a max of one is allowed." % self.__class__.__name__
        )
...
def get_anyof_instances(self, model_args, constant_args):
    ...
    if len(anyof_instances) == 0:
        raise ApiValueError(
            "Invalid inputs given to generate an instance of %s. Unable to "
            "make any instances of the classes in anyOf definition." % self.__class__.__name__
        )
    return anyof_instances
