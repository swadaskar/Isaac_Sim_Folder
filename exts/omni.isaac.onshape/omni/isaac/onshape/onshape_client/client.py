...
from omni.isaac.onshape.onshape_client.oas import ApiClient
from omni.isaac.onshape.onshape_client.oas import api
...
class Client:
    ...
    conf_file : :obj:`str`, optional
        The path to the location of an omni.isaac.onshape.onshape_client_config YAML file, where multiple stacks may be declared.
    ...
    add_defaults(
        ...
        client_id="omni.isaac.onshape.onshape_client_ID",
        client_secret="omni.isaac.onshape.onshape_client_SECRET",
    ...
    def __init__(
        self,
        keys_file="~/.omni.isaac.onshape.onshape_client_config.yaml",
        configuration=None,
        stack_key=None,
        ...
        if keys_file:
            ...
            update_with_default_carefully(
                final_configuration, Client._get_configuration_from_keys_file(keys_file, stack_key)
            )
    ...
    def fetch_access_token(self, **kwargs):
        ...
        """
        try:
            token_response = self.oauth.fetch_token(self.token_uri, client_secret=self.client_secret, **kwargs)
            self._set_oauth_creds_from_token_response(token_response)
            return token_response
        except Warning:
    ...
    def _create_apis(self):
        api_client = ApiClient(configuration=self.configuration, pool_threads=10)
