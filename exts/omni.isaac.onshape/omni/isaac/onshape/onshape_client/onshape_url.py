...
from omni.isaac.onshape.onshape_client.client import get_client
from omni.isaac.onshape.onshape_client.compatible_imports import parse, parse_qs
from omni.isaac.onshape.onshape_client.oas import *
from omni.isaac.onshape.onshape_client.units import u
...
class OnshapeElement(object):
    ...
    def import_file(self, file_path, translate=True, **kwargs):
        ...
        result = client.blob_elements_api.upload_file_create_element(
            self.did, self.wvmid, file=open(file_path, "rb"), encoded_filename=file_path.name, **kwargs
        )
        if translate:
    ...
    def poll_translation_result(translation_id):
        ...
        polling_function = lambda: get_client().translation_api.get_translation(translation_id)
        return OnshapeElement.poll(polling_function, is_polling_done)

    def export_file(self, file_path: Path, bt_translate_format_params: BTTranslateFormatParams):
        ...
        if self._get_element_info().data_type == OnshapeElement.DRAWING_DATA_TYPE:
            ...
            raise NotImplemented(f"Export for {self.element_type} through the OnshapeElement isn't supported yet.")
        result = func(**kwargs)
        if "store_in_document" in bt_translate_format_params.to_dict() and bt_translate_format_params.store_in_document:
            return
        ...
        if result.result_external_data_ids:
            ...
            for download_id in result.result_external_data_ids:
                file_path.write_bytes(
                    get_client()
                    .documents_api.download_external_data(did=self.did, fid=download_id, _preload_content=False)
                    .data
                )
                count = count + 1
    ...
    def mass_properties(self) -> BTMassPropertiesBulkInfo:
        return get_client().part_studios_api.get_part_studio_mass_properties(**self._get_DWMVE())
    ...
    def element_type(self):
        ...
        """
        elements = get_client().documents_api.get_elements_in_document(self.did, self.wvm, self.wvmid)
    ...
    def make_version(self, name, **kwargs):
        result = get_client().documents_api.create_version(
            self.did,
            bt_version_or_workspace_params=BTVersionOrWorkspaceParams(name=name, document_id=self.did, **kwargs),
            _preload_content=False,
        )
        return self.create_from_ids(wvmid=json.loads(result.data)["id"], wvm="v", sibling=self)
    ...
    def new_assembly(self, name="Assembly"):
        asm = get_client().assemblies_api.create_assembly(
            self.did, self.wvmid, BTModelElementParams(name=name), _preload_content=False
        )
    ...
    def _get_element_infos(self):
        return get_client().documents_api.get_elements_in_document(self.did, self.wvm, self.wvmid)
...
class ConfiguredOnshapeElement(OnshapeElement):
    ...
    def _get_configuration_encoding_response(self):
        res = get_client().elements_api.encode_configuration_map(
            self.did, self.eid, self._get_bt_configuration_params_for_current_configuration(), _preload_content=False
        )
    ...
    def _get_bt_configuration_params_for_current_configuration(self):
        ...
        for k, v in self.current_configuration.items():
            p = ConfigurationEntry(
                parameter_id=self._get_param_id_from_name(k), parameter_value=str(v).replace(" ", "+")
            )
            final.append(p)
