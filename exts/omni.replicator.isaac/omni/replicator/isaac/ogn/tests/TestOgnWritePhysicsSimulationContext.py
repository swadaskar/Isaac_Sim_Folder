...
class TestOgn(ogts.OmniGraphTestCase):
    ...
    async def test_data_access(self):
        from omni.replicator.isaac.ogn.OgnWritePhysicsSimulationContextDatabase import OgnWritePhysicsSimulationContextDatabase
        test_file_name = "OgnWritePhysicsSimulationContextTemplate.usda"
        usd_path = os.path.join(os.path.dirname(__file__), "usd", test_file_name)
        if not os.path.exists(usd_path):
            self.assertTrue(False, f"{usd_path} not found for loading test")
        (result, error) = await ogts.load_test_file(usd_path)
        self.assertTrue(result, f'{error} on {usd_path}')
        test_node = og.Controller.node("/TestGraph/Template_omni_replicator_isaac_OgnWritePhysicsSimulationContext")
        database = OgnWritePhysicsSimulationContextDatabase(test_node)
