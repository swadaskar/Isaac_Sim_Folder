...
class OgnROS2PublishImageDatabase(og.Database):
    ...
    class ValuesForInputs(og.DynamicAttributeAccess):
        LOCAL_PROPERTY_NAMES = {"context", "encoding", "execIn", "frameId", "height", "nodeNamespace", "queueSize", "timeStamp", "topicName", "width", "_setting_locked", "_batchedReadAttributes", "_batchedReadValues"}
        """Helper class that creates natural hierarchical access to input attributes"""
        def __init__(self, node: og.Node, attributes, dynamic_attributes: og.DynamicAttributeInterface):
            """Initialize simplified access for the attribute data"""
            context = node.get_graph().get_default_graph_context()
            super().__init__(context, node, attributes, dynamic_attributes)
            self._batchedReadAttributes = [self._attributes.context, self._attributes.encoding, self._attributes.execIn, self._attributes.frameId, self._attributes.height, self._attributes.nodeNamespace, self._attributes.queueSize, self._attributes.timeStamp, self._attributes.topicName, self._attributes.width]
            self._batchedReadValues = [0, "rgb8", None, "sim_camera", 0, "", 10, 0.0, "rgb", 0]
