"""Support for simplified access to data on nodes of type omni.isaac.ros_bridge.ROS1PublishBbox3D
...
import carb
import numpy
class OgnROS1PublishBbox3DDatabase(og.Database):
    """Helper class providing simplified access to data on nodes of type omni.isaac.ros_bridge.ROS1PublishBbox3D
    ...
    INTERFACE = og.Database._get_interface([
        ...
        ('inputs:topicName', 'string', 0, None, 'Name of ROS1 Topic', {ogn.MetadataKeys.DEFAULT: '"bbox3d"'}, True, 'bbox3d', False, ''),
    ])
    @classmethod
    ...
    class ValuesForInputs(og.DynamicAttributeAccess):
        ...
        def __init__(self, node: og.Node, attributes, dynamic_attributes: og.DynamicAttributeInterface):
            ...
            self._batchedReadValues = [None, "sim_camera", "", 10, 0.0, "bbox3d"]
    ...
    def __init__(self, node):
        ...
        self.inputs = OgnROS1PublishBbox3DDatabase.ValuesForInputs(node, self.attributes.inputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_OUTPUT)
        self.outputs = OgnROS1PublishBbox3DDatabase.ValuesForOutputs(node, self.attributes.outputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_STATE)
        self.state = OgnROS1PublishBbox3DDatabase.ValuesForState(node, self.attributes.state, dynamic_attributes)
