"""Support for simplified access to data on nodes of type omni.isaac.ros_bridge.ROS1PublishPointCloud
...
import numpy
import carb
class OgnROS1PublishPointCloudDatabase(og.Database):
    """Helper class providing simplified access to data on nodes of type omni.isaac.ros_bridge.ROS1PublishPointCloud
    ...
    INTERFACE = og.Database._get_interface([
        ...
        ('inputs:topicName', 'string', 0, None, 'Name of ROS1 Topic', {ogn.MetadataKeys.DEFAULT: '"point_cloud"'}, True, 'point_cloud', False, ''),
    ])
    @classmethod
    ...
    class ValuesForInputs(og.DynamicAttributeAccess):
        LOCAL_PROPERTY_NAMES = {"execIn", "frameId", "nodeNamespace", "queueSize", "timeStamp", "topicName", "_setting_locked", "_batchedReadAttributes", "_batchedReadValues"}
        """Helper class that creates natural hierarchical access to input attributes"""
        def __init__(self, node: og.Node, attributes, dynamic_attributes: og.DynamicAttributeInterface):
            """Initialize simplified access for the attribute data"""
            context = node.get_graph().get_default_graph_context()
            super().__init__(context, node, attributes, dynamic_attributes)
            self._batchedReadAttributes = [self._attributes.execIn, self._attributes.frameId, self._attributes.nodeNamespace, self._attributes.queueSize, self._attributes.timeStamp, self._attributes.topicName]
            self._batchedReadValues = [None, "sim_lidar", "", 10, 0.0, "point_cloud"]
    ...
    def __init__(self, node):
        ...
        self.inputs = OgnROS1PublishPointCloudDatabase.ValuesForInputs(node, self.attributes.inputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_OUTPUT)
        self.outputs = OgnROS1PublishPointCloudDatabase.ValuesForOutputs(node, self.attributes.outputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_STATE)
        self.state = OgnROS1PublishPointCloudDatabase.ValuesForState(node, self.attributes.state, dynamic_attributes)
