...
class OgnROS1CameraHelperDatabase(og.Database):
    ...
    class ValuesForInputs(og.DynamicAttributeAccess):
        LOCAL_PROPERTY_NAMES = {"enableSemanticLabels", "execIn", "frameId", "nodeNamespace", "queueSize", "renderProductPath", "resetSimulationTimeOnStop", "semanticLabelsTopicName", "stereoOffset", "topicName", "type", "viewport", "_setting_locked", "_batchedReadAttributes", "_batchedReadValues"}
        """Helper class that creates natural hierarchical access to input attributes"""
        def __init__(self, node: og.Node, attributes, dynamic_attributes: og.DynamicAttributeInterface):
            """Initialize simplified access for the attribute data"""
            context = node.get_graph().get_default_graph_context()
            super().__init__(context, node, attributes, dynamic_attributes)
            self._batchedReadAttributes = [self._attributes.enableSemanticLabels, self._attributes.execIn, self._attributes.frameId, self._attributes.nodeNamespace, self._attributes.queueSize, self._attributes.renderProductPath, self._attributes.resetSimulationTimeOnStop, self._attributes.semanticLabelsTopicName, self._attributes.stereoOffset, self._attributes.topicName, self._attributes.type, self._attributes.viewport]
            self._batchedReadValues = [False, None, "sim_camera", "", 10, "", False, "semantic_labels", [0, 0], "rgb", "rgb", ""]
